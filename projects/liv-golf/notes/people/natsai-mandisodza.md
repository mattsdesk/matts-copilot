# Natsai Mandisodza

**Role:** Product Manager
**Organisation:** Fantasy Co (LatitudeGo)
**Relationship:** Vendor / Partner
**Context:** LIV Golf
**Last interaction:** March 17, 2026

---

## Context

Natsai is a product manager at Fantasy Co working on the web platform rebuild and fantasy game features. She's involved in the project coordination, deck preparation, and stakeholder communication around web launch timeline and complexity.

## Key Themes

- **Web project coordination** — Contributing to project planning and timeline discussions
- **Stakeholder communication** — Supporting deck preparation and complexity explanation for executives
- **Product delivery** — Part of the Fantasy Co team managing feature delivery and requirements clarity

## Talking Points
*Topics to raise in next interaction.*

## Open Items

- Dev URL sharing for South Africa parallel testing (LIV team side-by-side)
- Scope alignment doc due end of week (March 20)
- URL redirect mapping due April 3

## 1:1 Notes

*Running log of key points from each 1:1 — most recent first.*

## Interaction History

| Date | Meeting | Notes |
|------|---------|-------|
| 2026-03-17 | [[EXT] Project Apex Production Sync](../meetings/2026/2026-03-17-project-apex-production-sync.md) | South Africa testing alignment; Mexico launch date disagreement; URL redirect mapping; invoice issues |
| 2026-03-10 | [Sync on Updated Deck](../meetings/2026/2026-03-10-sync-on-updated-deck.md) | Web launch timeline, complexity factors, team resources |

---

*Part of the [notes repo](../README.md)*
